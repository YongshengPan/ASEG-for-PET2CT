...
def main_task(model_stats='extra_test',
    ...
    synAB_template = {'network': synthesize_backbone[0],
                      ...
                      'task_type': 'synthesis', 'activation': 'elu',
...
def squence_task(model_stats, parameters, devices=0):
        ...
        if parameters['task_model'] == 'specific':
                ...
                combB_constraints = comb_constraints + ('ct_dice',)
        ...
        restore_models = {'clsA': None, 'clsB': None, 'synA': 150, 'synB': 150, 'advA': 150, 'advB': 150}
        # restore_models = {'clsA': None, 'clsB': None, 'synA': None, 'synB': None, 'advA': None, 'advB': None}
...
if __name__ == '__main__':
    ...
    costraints = ('dis', 'p2p', 'msl')
    # costraints = ('p2p', )
    allcostraints = list(combinations(costraints, 3)) + list(combinations(costraints, 2)) + list(combinations(costraints, 1))
    # allcostraints = [('dice', 'cre'), ]
    # allcostraints = [cons for cons in allcostraints if 'p2p' in cons]

    param_essambles = [#[{'views': angview, 'costraints': costraint, 'task_model': 'general'} for angview in angle_of_views[0:1] for costraint in allcostraints[3:4]],
                       #[{'views': angview, 'costraints': ('cre', 'dice'), 'task_model': 'segment'} for angview in angle_of_views],
                       [{'views': angview, 'costraints': costraint, 'task_model': 'specific'} for angview in angle_of_views[0:1] for costraint in allcostraints[3:4]],
                       [{'views': angview, 'costraints': costraint, 'task_model': 'segboost'} for angview in angle_of_views[0:1] for costraint in allcostraints[3:4]],
                       #[{'views': angview, 'costraints': costraint, 'task_model': 'collabrate'} for angview in angle_of_views[0:1] for costraint in allcostraints],
                       #[{'views': angview, 'costraints': costraint + ('sin',), 'task_model': 'collabrate'} for angview in angle_of_views[0:1] for costraint in allcostraints[0:1]],
                       #[{'views': angview, 'costraints': costraint + ('sin',), 'task_model': 'general'} for angview in angle_of_views[0:1] for costraint in allcostraints[0:1]],
                       #[{'views': angview, 'costraints': costraint + ('maxp',), 'task_model': 'specific'} for angview in angle_of_views[0:1] for costraint in allcostraints],
                       #[{'views': angview, 'costraints': costraint + ('maxp',), 'task_model': 'general'} for angview in angle_of_views[0:1] for costraint in allcostraints],
                        ]
