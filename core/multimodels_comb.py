...
import threading
import numpy as np
from .measurement_threading import matricsThread
...
class MultiModels(object):
    def __init__(self, database,
        ...
        self.modelmap = {'simpleunet': SimpleEncoderDecoder,
                         ...
                         'affineregister': SimpleAffineRegister,
        ...
        for idx in range(len(self.database.train_combinations)):
            for modname in network_modules:
                if modname in modules:
                    network_modules[modname][idx].save_weights(self.chkpt_cls_fname + '/clsA_model{0}_{1}.h5'.format(idx, globel_epoch))
    ...
    def model_load(self, globel_epoch=0, modules=('cls', 'syn'), by_name=False, skip_mismatch=False):
        ...
        for idx in range(len(self.database.train_combinations)):
            for modname in ['clsA', 'clsB', 'synA', 'synB', 'advA', 'advB']:
                if modname in modules:
                    network_modules[modname][idx].load_weights(self.chkpt_cls_fname + '/{0}_model{1}_{2}.h5'.format(modname, idx, globel_epoch),
    ...
    def iteration_modules_v2(self, inputs, cv_index,
        ...
        weight_mask = 0.5 #if 'mask' not in inputs else inputs['mask']
        if 'synA' in modules:
            with tf.GradientTape(persistent=False, watch_accessed_variables=False) as tape:
        ...
        if self.synB_params['use_fake'] > 0 and synA_prob is not None and synB_alter is not None:
            synB_logit, synB_prob, _ = self.synBs[cv_index](kconcat([synA_prob, synA_const], axis=-1))
            # synB_logit, synB_prob, _ = self.synBs[cv_index](kconcat([tf.nn.sigmoid(synA_prob*100-50), synB_const], axis=-1))
            # attributes.update({'synA': synA_prob.numpy()})
            attributes.update({'synB': synB_prob.numpy()})
    ...
    def iteration_modules_v2_inference(self, inputs, cv_index,
                             modules=('clsA', 'clsB', 'synA', 'synB', 'advA', 'advB'), outkeys=None, flags=None):
    ...
    def test_synthesis(self):
        ...
        for ptr in range(0, min(self.max_num_images, nums_of_samps)):
            ...
            if load_pregenerated:
                ...
                outkeys = ['flnm'] + ['synA', 'refA', 'synB', 'refB']
                ...
                if True:
                    ...
                    for fctr in range(50):
                        ...
                        fuse_weightX = 1 - np.abs((np.arange(sX1, sX2) - (sX1 + sX2-1) / 2) / (sX2 - sX1))*1.5
                        fuse_weightY = 1 - np.abs((np.arange(sY1, sY2) - (sY1 + sY2-1) / 2) / (sY2 - sY1))*1.5
                        fuse_weightZ = 1 - np.abs((np.arange(sZ1, sZ2) - (sZ1 + sZ2-1) / 2) / (sZ2 - sZ1))*1.5
                        fuse_weight = np.meshgrid(fuse_weightX, fuse_weightY, fuse_weightZ, indexing='ij')
                        fuse_weight = np.expand_dims(np.prod(fuse_weight, axis=0), axis=-1)
                        ref_A[sX1:sX2, sY1:sY2, sZ1:sZ2] += attrs['refA'][0]*fuse_weight
                        ref_B[sX1:sX2, sY1:sY2, sZ1:sZ2] += attrs['refB'][0]*fuse_weight
                        syn_A[sX1:sX2, sY1:sY2, sZ1:sZ2] += attrs['synA'][0]*fuse_weight
                        syn_B[sX1:sX2, sY1:sY2, sZ1:sZ2] += attrs['synB'][0]*fuse_weight
                        cusum_A[sX1:sX2, sY1:sY2, sZ1:sZ2] += fuse_weight
                        cusum_B[sX1:sX2, sY1:sY2, sZ1:sZ2] += fuse_weight
                        if fctr >= mm_images['count_of_augs'][0]: break
                    synA, synB = syn_A/cusum_A, syn_B/cusum_B
                    refA, refB = ref_A/cusum_A, ref_B/cusum_B
                    synA = np.where(cusum_A > 0.01, synA, np.min(synA[cusum_A > 0.01]))
                    synB = np.where(cusum_B > 0.01, synB, np.min(synB[cusum_B > 0.01]))
                    refA = np.where(cusum_A > 0.01, refA, np.min(refA[cusum_A > 0.01]))
                    refB = np.where(cusum_B > 0.01, refB, np.min(refB[cusum_B > 0.01]))
                else:
                ...
                save_thread = threading.Thread(target=self.database.save_output, args=(self.result_path, flnm, eval_out))
                save_thread.start()
                #self.database.save_output(self.result_path, flnm, eval_out)

            if refA is not None:
                # thread_Ametrics = matricsThread(synA/np.mean(synA), refA/np.mean(refA), task_type=self.synA_params['task_type'], isinst=True, data_range=2.0, name='refA')
                thread_Ametrics = matricsThread(synA, refA, task_type=self.synA_params['task_type'], isinst=True, data_range=2.0, name='refA')
            else:
                thread_Ametrics = threading.Thread()

                print('synA: ', flnm,  syn_metrics)
                synA_metrics.append(syn_metrics)
            if refB is not None:
                # thread_Bmetrics = matricsThread(synB/np.mean(synB), refB/np.mean(refB), task_type=self.synB_params['task_type'], isinst=True, data_range=2.0, name='refB')
                thread_Bmetrics = matricsThread(synB, refB, task_type=self.synB_params['task_type'], isinst=True, data_range=2.0, name='refB')
            else:
                thread_Bmetrics = threading.Thread()
            thread_Ametrics.start()
            thread_Bmetrics.start()
            thread_Ametrics.join()
            thread_Bmetrics.join()
            if refA is not None:
                syn_metrics = thread_Ametrics.get_output()
                print('synA: ', flnm, syn_metrics)
                synA_metrics.append(syn_metrics)
            if refB is not None:
                syn_metrics = thread_Bmetrics.get_output()
